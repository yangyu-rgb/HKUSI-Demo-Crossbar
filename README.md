...
- `/model` 汇总 V1 合成数据指标、运行时影子差异、完整 Demo 就绪度和独立的 V2 数据门槛。
...
AI v1 保留为合成数据影子对照。课堂 AI v2 使用覆盖天气、事件、方向和时间的可解释合成场景数据训练，FastAPI 默认用它预测等待分钟并驱动路线建议，产物不可用时回退统计模型。两代模型的指标都不代表真实口岸效果。

V2 采用真实数据优先策略。生成数据继续用于训练流水线回归、极端情景测试和消融实验，但不会进入真实验证集、测试集或晋级指标。众包记录只有在明确声明为实际现场反馈、获得建模同意、关联到预测运行且质量达标时，才会进入 V2 训练快照；演示种子和普通 Demo 录入始终隔离。
...
本清单按优先级而非日期维护。**V1 可执行 Demo 的整体逻辑闭环已经完成**；当前下一项工作是持续积累来源可追溯、获得建模同意的高质量真实预测—反馈标签。只有达到 200 条并覆盖四个口岸、21 个日期、8 个小时切片且时间切分可用后，才可开展受控 V2 实验。

V1 Demo 有意保留的边界：

- 地点、交通、天气和口岸状态仍来自本地模拟输入。
- 身份、组织、通知、数据库与审计均为本地适配器，不等同于生产认证、外部通知或生产基础设施。
- AI v2 场景模型直接预测四口岸等待时间并驱动用户结果；产物不可用时自动降级到可解释统计模型。AI v1 保留为影子对照。
- 香港入境处官方拥堵等级和每日客流进入隔离的真实特征区，不会被转换成等待分钟标签。
...
- [x] 持久化提醒评估历史和已读状态
- [x] 为预测生成稳定运行 ID，并把高质量众包反馈关联为实际等待标签
- [x] 导出可审计训练快照，并按真实标签、口岸、日期和时间切片检查 V2 就绪度
- [x] 隔离演示/生成数据，并记录真实反馈方向、通道、时间范围、来源和建模同意
...
- [x] 设计口岸、天气、日历、事件和众包种子 Provider 接口
- [x] 实现本地 JSON 不可用时的内嵌模拟数据降级和来源元数据
- [x] 为 V1 Demo 实现本地通知、身份、组织隔离、健康检查和审计适配器
- [x] 增加 GitHub Actions 的后端、前端、契约和 Chromium 端到端验证
- [x] 建立真实数据源审批登记，并接入香港官方拥堵等级与每日客流特征采集
- [x] 增加原始响应归档、采集质量/覆盖审计和精确标签只读校验
- [x] 保存官方特征修订历史，并按预测生成时刻冻结可复现的特征快照
- [x] 增加官方等级一致性评估、24 小时采集完整率和本地采集 sidecar
- [ ] 取得可用于分钟回归的合法精确等待标签来源
- [ ] 完成深圳开放数据字段/许可核验并取得 i口岸书面授权（未授权前保持禁用）
- [ ] 评估生产预测模型和真实数据回测方法
- [ ] 接入生产邮件、短信或推送通知服务
- [ ] 接入生产用户认证与多租户权限系统
...
- [V2 开发前置检查](docs/v2_readiness.md) - 标签闭环、训练快照与晋级门槛
- [真实数据源准入清单](docs/real_data_intake.md) - 授权、隐私、语义和质量审查流程
- [V1 Demo 完成标准](docs/v1_demo_readiness.md) - 功能闭环、验证入口与生产边界
- [官方数据源与采集](docs/official_data_sources.md) - 来源审批、字段映射、采集命令和标签边界
