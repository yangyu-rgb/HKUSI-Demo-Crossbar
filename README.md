...
本清单按优先级而非日期维护。当前下一项功能是**智能提醒评估与预览**。

仍待替换的主要固定逻辑：

- 提醒时间目前只按到达时间固定减 75 分钟，尚未执行真实订阅评估。
- 企业方案按员工序号选择路线偏好，而不是使用明确的企业或员工配置。
- 地点、交通、天气和口岸状态仍来自本地模拟输入。
- 重复事件配置尚未参与预测，节假日数据当前为空。
...
- [ ] 计算订阅下一次有效通勤日期和到达时间
- [ ] 调用预测服务计算订阅的推荐口岸与最晚出发时间
- [ ] 生成出发前 30 分钟提醒预览
- [ ] 生成异常拥堵和备选路线提醒预览
- [ ] 增加提醒评估 API、前端预览区域和自动化测试
- [ ] 移除企业方案按员工序号决定偏好的固定逻辑
- [ ] 为企业员工或整批方案提供明确的偏好与预算输入
- [ ] 完成核心 Demo 全流程回归和重置验证

### 中优先级：质量与演示完善

- [ ] 将节假日和重复事件配置实际接入预测因子
- [ ] 支持订阅七天选择和三个提醒开关的真实编辑
- [ ] 持久化提醒评估历史和已读状态
