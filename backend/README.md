...
`/api/enterprise-operations/*` 实际调用已校验的 AI v2.2 HGB 产物生成各口岸基础等待，以重建容量压力做透明校准，再由确定性约束层生成调度预览；响应通过 `ai_decision_trace` 暴露模型版本、输入、90% 区间和优化目标。采用方案、通知草稿、协调建议和人工复盘保存在 SQLite。它不会连接真实巴士、货车、短信、客服或口岸系统；等待标签、成本暴露和改善指标仍是课堂重建情景。
