...
- `/planner`：登录后可用的预测表单、可解释结果和 SVG 路线。
...
首页同时提供懒加载 Three.js 四路线流动压力动画与 SVG 2D 地图。移动端提供返回网页版、PWA manifest、生产构建安装入口、在线状态和离线壳；离线时不会缓存或伪造预测 API 响应。企业实时全屏大屏仍保留在 README TODO。
