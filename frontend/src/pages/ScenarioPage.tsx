...
export function ScenarioPage() {
  const { scenarios, save, restore, reset } = useScenarios();
  ...
  if (scenarios.isPending) return <PageSkeleton cards={3} />;
  if (!scenarios.data || !draft || !selected) return <ErrorState title="无法载入场景实验室" detail={userFacingError(scenarios.error)} />;
