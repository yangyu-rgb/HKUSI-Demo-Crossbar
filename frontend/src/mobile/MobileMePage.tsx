...
export function MobileMePage() {
  ...
  return (
    <main className={styles.page}>
      ...
      {tab === "model" && <><section className={styles.resultHero}><small>AI v2.2 · public data hybrid classroom demo</small><h2>{model.data?.artifact_available ? "主预测已启用" : "统计模型降级"}</h2><p>香港官方客流负责基础特征，深圳官方快照只用于交叉核验区间。</p><div className={styles.metrics}><div><strong>{metrics?.test?.mae ?? 1.1368}</strong><span>测试 MAE</span></div><div><strong>{interval?.test_coverage_percent ?? 90.44}%</strong><span>区间覆盖</span></div><div><strong>45%</strong><span>高共识上限</span></div></div></section><section className={styles.card}><h2>等待时间怎样算</h2><div className={styles.list}><article><strong>1. AI 学习基础等待</strong><p>按口岸、方向、时间和香港官方客流压力生成底数。</p></article><article><strong>2. 透明校准场景</strong><p>天气、节假日、活动、官方等级和新鲜众包逐步修正。</p></article><article><strong>3. 深圳快照核验区间</strong><p>两侧压力差异越大，预测范围越保守，不重复计算旅客。</p></article></div><p className={styles.message}>所有分钟数均为课堂 Demo 估算，不是现场实测或生产准确率声明。</p></section></>}
    </main>
  );
}
